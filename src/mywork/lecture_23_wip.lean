...
def anti_reflexive := ∀ x, ¬ x ≺ x --not the same as not reflexive because reflexive means everything is related to itself while anti reflexive means everything is not related to itself. Not reflexive means not everything is related to itself
def irreflexive := anti_reflexive r -- sometimes used 
def anti_symmetric := ∀ ⦃x y⦄, x ≺ y → y ≺ x → x = y -- less than or equal 
def asymmetric := ∀ ⦃x y⦄, x ≺ y → ¬ y ≺ x -- less than 
...
-- symmetric not everything needs to be connected, but what is connected must be connected both ways
-- the TC or Transitive Closure is the smallest relation r ⊆ r' such that r is a subrelation and r' is transitive. 

example : reflexive r → ¬ asymmetric r := _  -- true?
example : ¬ reflexive r ↔ irreflexive r := _  -- true? flase 
