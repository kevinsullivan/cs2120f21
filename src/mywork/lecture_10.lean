...
--ex false quod libet or from false anything follows, use this when doing english proofs with false.elim
begin
  assume P,
  assume f,
  cases f,
...
theorem no_true_and_false : ∀ (p : Prop), ¬p ∨ p := 
begin
  assume P p,
  cases p,
end
-- ^ there is no proof in lean for this, we have to add an axiom

--goldbock conjector said for all even numbers greater then 2 is the sum of two primes
--this has been tested for numbers 1 * 10^18 with computers. For natural numbers
--we do not have a proof for all even numbers
--that is why it is called a conjecture 
theorem no_contradiction : ∀ (P : Prop), ¬(P ∧ ¬P) :=
begin
  assume p q,
  have p1 : p := and.elim_left q, --could also use q.left
  have p2 : ¬p := and.elim_right q, -- could also use q.right
  apply not.intro p2, -- not p is a machine that turns a proof of p into a proof that p implies false
  apply p1,
end
...
axioms truth : ∀ (P : Prop), (P ∨ ¬P) -- we can add this axiom 
