...
/-
memorize all defenitions in this file -> it is essential !!!
...
--this lets us avoid having to right for all types α or β 
...
∀ ⦃a : α⦄, a ∈ s₁ → a ∈ s₂
-- for all a of type α, if a is a member of s1 then it is a member of s2. 

--when ever you don't understand something in math go look at the defenition 

-- can you prove a subset by taking the power of the original set and then showing the subset is in the power set
...
-- sep primes evens -> this would give us a set of evens 
/-
...
-- lambda symbol comes from functional programming and type theory - since the 30s
-- built in to lots of languages. It is a function without a name, anonymous function
-- 
...
{a | a ∈ s ∧ a ∉ t}
--{ a | a ∈ s ∧ a ∉ t }
--must memorize these defenitions, practice by copying the defenition out and writting from scratch
/-
Powerset
-/
...
-- the set of results, for inputs of a to function f we get a new set of outputs. applying a function to each value in the set
-- lifting a function means a function of individual values can become a function for sets. 
/-
