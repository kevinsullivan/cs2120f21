...
/-
Universal quantification for all in an empty set
is always true and universal exsistense in an empty
set is always false. 

∀ in a set is ∧ for all elements
∃ in a set is ∨ for all elements 
-/
...
reflexive? no - it is actually irreflexive
...
counterexample of transitive and symmetric → reflexive
-/
...
Closure adds the smallest number of new relations
to the existing relations to make the new closure
true. i.e. adding self arrows to all objects
