...
(n : nat),
motive nat.zero → 
(Π (n' : nat), motive n' → motive n'.succ) → motive n
...
exact n_n + n_ih + 1,              -- suppose n' is arbitrary
...
begin
  ...
  induction n with nPrime nPrimeanswer,

  exact 1,

  exact (nPrime + 1) * nPrimeanswer,
...
Induction for natural number

∀ (n : nat), P n,
P 0
∀ n', p n' → n'+1 -- also n.succ
∀ n, P n
