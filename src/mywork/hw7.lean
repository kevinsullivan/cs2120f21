...
example : (∃ (b : β), true) → asymmetric r → ¬reflexive r := --this is wrong when considering the empty set until we add that some beta b exists
begin
  unfold asymmetric reflexive,
  assume ex assym,
  assume refl,  
  cases ex with b truth,
  have f1 := refl b,
  have f2 := assym f1,
  contradiction,
end
...
-/
example : (∃ (b : β), true) → transitive r → reflexive r → ¬ asymmetric r := --same as above, if you don't have beta it is not true. I added the condition that their exists some beta
begin
  unfold transitive reflexive asymmetric,
  assume beta trans refl asym,
  cases beta with b t,
  have f1 := refl b,
  have f2 := asym f1,
  contradiction,
